namespace Utilities.File.Mime
{
    ...
    public static class MimeTypes
    {
        ...
        static MimeTypes()
        {
            types = new List<FileType> {PDF, WORD, EXCEL, JPEG, ZIP, RAR, HTML, RTF, PNG, PPT, GIF, DLL_EXE, MSDOC,
        ...
        public static FileType GetFileType(Func<byte[]> fileHeaderReadFunc, string fileFullName = "")
        {
            ...
            {
                int matchingCount = SearchBytes(fileHeader, Enumerable.ToArray(Enumerable.Select(type.Header, x => x.GetValueOrDefault())));
                if (matchingCount != -1)
                {
