...
namespace HtmlRenderer.Core.Core.Dom
{
    ...
    internal sealed class CssLayoutEngineTable
    {
        ...
        private void EnforceMinimumSize()
        {
            foreach (CssBox row in this._allRows)
            {
                foreach (CssBox cell in row.Boxes)
                {
                    ...
                    if (this._columnWidths.Length > col && this._columnWidths[col] < GetColumnMinWidths()[col])
                    {
                        double diff = GetColumnMinWidths()[col] - this._columnWidths[col];
                        this._columnWidths[affectcol] = GetColumnMinWidths()[affectcol];
        ...
        {
            double startx = Math.Max(this._tableBox.ClientLeft + GetHorizontalSpacing(), 0);
            double starty = Math.Max(this._tableBox.ClientTop + GetVerticalSpacing(), 0);
            double cury = starty;
            ...
            if (this._tableBox.TextAlign == CssConstants.Center || this._tableBox.TextAlign == CssConstants.Right)
            {
                ...
                this._tableBox.Location = new RPoint(startx - this._tableBox.ActualBorderLeftWidth - this._tableBox.ActualPaddingLeft - GetHorizontalSpacing(), this._tableBox.Location.Y);
            }
            ...
            this._tableBox.ActualRight = maxRight + GetHorizontalSpacing() + this._tableBox.ActualBorderRightWidth;
            this._tableBox.ActualBottom = Math.Max(maxBottom, starty) + GetVerticalSpacing() + this._tableBox.ActualBorderBottomWidth;
        }
        ...
        {
            if (this._columnWidths.Length >= columnIndex || GetColumnMinWidths().Length >= columnIndex)
                return false;
            return this._columnWidths[columnIndex] > GetColumnMinWidths()[columnIndex];
        }
        ...
        {
            return GetAvailableTableWidth() - GetHorizontalSpacing() * (this._columnCount + 1) - this._tableBox.ActualBorderLeftWidth - this._tableBox.ActualBorderRightWidth;
        }
        ...
        private double GetWidthSum()
        {
            ...
            //Take cell-spacing
            f += GetHorizontalSpacing() * (this._columnWidths.Length + 1);
