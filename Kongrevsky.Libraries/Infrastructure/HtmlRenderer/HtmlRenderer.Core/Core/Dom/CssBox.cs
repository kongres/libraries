...
namespace HtmlRenderer.Core.Core.Dom
{
    ...
    internal class CssBox : CssBoxProperties, IDisposable
    {
        ...
        public void ParseToWords()
        {
            ...
            while (startIdx < this._text.Length)
            {
                ...
                if (startIdx < this._text.Length)
                {
                    ...
                    {
                        endIdx = startIdx;
                        while (endIdx < this._text.Length && !char.IsWhiteSpace(this._text[endIdx]) && this._text[endIdx] != '-' && WordBreak != CssConstants.BreakAll && !CommonUtils.IsAsianCharecter(this._text[endIdx]))
                            endIdx++;

                        if (endIdx < this._text.Length && (this._text[endIdx] == '-' || WordBreak == CssConstants.BreakAll || CommonUtils.IsAsianCharecter(this._text[endIdx])))
                            endIdx++;
        ...
        internal virtual void MeasureWordsSize(RGraphics g)
        {
            ...
            {
                if (BackgroundImage != CssConstants.None && this._imageLoadHandler == null)
                {
                    this._imageLoadHandler = new ImageLoadHandler(HtmlContainer, OnImageLoadComplete);
                    this._imageLoadHandler.LoadImage(BackgroundImage, HtmlTag != null ? HtmlTag.Attributes : null);
                }
        ...
        private void CreateListItemBox(RGraphics g)
        {
            if (Display == CssConstants.ListItem && ListStyleType != CssConstants.None)
            {
                if (this._listItemBox == null)
                {
                    ...
                    this._listItemBox.HtmlContainer = HtmlContainer;
                    ...
                    {
                        this._listItemBox.Text = new SubString(GetIndexForList().ToString(CultureInfo.InvariantCulture) + ".");
                    }
                    else if (ListStyleType.Equals(CssConstants.DecimalLeadingZero, StringComparison.InvariantCultureIgnoreCase))
                    {
                        this._listItemBox.Text = new SubString(GetIndexForList().ToString("00", CultureInfo.InvariantCulture) + ".");
                    }
                    else
                    {
                        this._listItemBox.Text = new SubString(CommonUtils.ConvertToAlphaNumber(GetIndexForList(), ListStyleType) + ".");
                    }
                ...
                this._listItemBox.Words[0].Left = Location.X - this._listItemBox.Size.Width - 5;
                this._listItemBox.Words[0].Top = Location.Y + ActualPaddingTop; // +FontAscent;
            }
        }
        ...
        protected double MarginTopCollapse(CssBoxProperties prevSibling)
        {
            ...
            else if (this._parentBox != null && ActualPaddingTop < 0.1 && ActualPaddingBottom < 0.1 && this._parentBox.ActualPaddingTop < 0.1 && this._parentBox.ActualPaddingBottom < 0.1)
            {
                value = Math.Max(0, ActualMarginTop - Math.Max(this._parentBox.ActualMarginTop, this._parentBox.CollapsedMarginTop));
            }
            else
            {
        ...
        {
            double margin = 0;
            if (ParentBox != null && ParentBox.Boxes.IndexOf(this) == ParentBox.Boxes.Count - 1 && this._parentBox.ActualMarginBottom < 0.1)
            {
            ...
            return Math.Max(ActualBottom, this._boxes[this._boxes.Count - 1].ActualBottom + margin + ActualPaddingBottom + ActualBorderBottomWidth);
        }
